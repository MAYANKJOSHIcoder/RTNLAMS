...
export default function RiskAlerts({ assessments, parcels }: RiskAlertsProps) {
  const top = [...assessments]
    ...
    .slice(0, 5);
  if (!top.length) return <div className="bg-white border border-slate-200 rounded-xl p-6 text-center text-sm text-slate-500">No high-risk alerts</div>;
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-4">
      ...
      <div className="space-y-2">
        {top.map((a) => {
          ...
          return (
            <div key={a.parcel_id} className="flex items-center gap-3 p-2.5 border border-slate-200 rounded-lg hover:bg-slate-50">
